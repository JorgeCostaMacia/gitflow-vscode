...
/** Use case: finish a feature by merging it into develop. */
export class FeatureFinishService {
  ...
  /** Merges `feature/<name>` into develop and deletes the local and remote branch. */
  async execute(name: string): Promise<void> {
    ...
    // Delete the remote branch first so `git branch -d` does not refuse the
    // local one for being "not merged to its upstream".
